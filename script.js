...
const inner = document.querySelector('.register__inner');
...
const anim = setInterval(() => {
    inner.classList.toggle('animation');
...
function registerForm () {
    ...
    if (name == user.name && password == user.password) {
        h2.innerText = `login ${name} используется`;
        user.name = '';
    } 
    ...
    if (rus >= 1 || letterBig < 3 || symbol < 2) {
        h2.innerText = 'password bad';
        user.name = '';
        user.password = '';
        passwordForm.classList.toggle('wrong-password');
    } else {
        h1.innerText = "autorisation";
    }
